...
/** Basic implementation of knapsack solution for talks that returns
  ...
  */
object KnapsackSolutionForTalks2 {
  ...
  def apply(goal: Duration)(talks: Set[Talk]): OnMetReq[TalksCombinations] =
  ...
  private def apply(talks: Set[Talk], goal: Duration): OnMetReq[WCombinations] = {
  ...
  private def adaptForGeneralSolution(goal: Duration, talks: Set[Talk]) =
  ...
  private def simplify(x: WCombination): Set[Talk] = x.map(_.value).toSet
